...
def get_registration_otp(verification_id):
    conn = get_connection()
    row = conn.execute("SELECT * FROM registration_otps WHERE id = ?", (verification_id,)).fetchone()
    conn.close()
    return dict(row) if row else None


def increment_registration_otp_attempts(verification_id):
    conn = get_connection()
    conn.execute("UPDATE registration_otps SET attempts = attempts + 1 WHERE id = ?", (verification_id,))
    conn.commit()
    conn.close()


def delete_registration_otp(verification_id):
    conn = get_connection()
    conn.execute("DELETE FROM registration_otps WHERE id = ?", (verification_id,))
    conn.commit()
    conn.close()


def create_mobile_otp(phone, otp_hash, expires_at, last_sent_at=None):
    normalized = normalize_indian_mobile(phone)
    if not normalized:
        return None
    conn = get_connection()
    existing = conn.execute("SELECT id FROM mobile_otps WHERE phone = ?", (normalized,)).fetchone()
    if existing:
        conn.execute(
            "UPDATE mobile_otps SET otp_hash = ?, expires_at = ?, attempts = 0, resent_count = resent_count + 1, last_sent_at = ? WHERE phone = ?",
            (otp_hash, expires_at, last_sent_at or datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'), normalized),
        )
        record_id = existing[0]
    else:
        cursor = conn.execute(
            "INSERT INTO mobile_otps (phone, otp_hash, expires_at, attempts, resent_count, last_sent_at) VALUES (?, ?, ?, 0, 1, ?)",
            (normalized, otp_hash, expires_at, last_sent_at or datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')),
        )
...
def get_mobile_otp(phone):
    normalized = normalize_indian_mobile(phone)
    if not normalized:
        return None
    conn = get_connection()
    row = conn.execute("SELECT * FROM mobile_otps WHERE phone = ?", (normalized,)).fetchone()
    conn.close()
    return dict(row) if row else None


def increment_mobile_otp_attempts(phone):
    normalized = normalize_indian_mobile(phone)
    if not normalized:
        return
    conn = get_connection()
    conn.execute("UPDATE mobile_otps SET attempts = attempts + 1 WHERE phone = ?", (normalized,))
    conn.commit()
    conn.close()


def delete_mobile_otp(phone):
    normalized = normalize_indian_mobile(phone)
    if not normalized:
        return
    conn = get_connection()
    conn.execute("DELETE FROM mobile_otps WHERE phone = ?", (normalized,))
    conn.commit()
    conn.close()


def update_mobile_otp_last_sent(phone, timestamp):
    normalized = normalize_indian_mobile(phone)
    if not normalized:
        return
    conn = get_connection()
    conn.execute("UPDATE mobile_otps SET last_sent_at = ? WHERE phone = ?", (timestamp, normalized))
    conn.commit()
    conn.close()
